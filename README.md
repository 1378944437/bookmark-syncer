...
书签是私密的数字资产。**MarkSync** 彻底摒弃中心化中转服务器，利用你自己的 **WebDAV 云盘**（坚果云、Nextcloud、群晖 NAS、InfiniCLOUD、Alist 或自建云），在 **Chrome、Edge、Firefox** 及移动端之间实现安全、快速、自主的双向书签同步。

```
┌─────────────────┐       端到端加密通道 (TLS + AES-256-GCM)       ┌────────────────────────┐
│  本地浏览器      │ ◄────────────────────────────────────────────► │ 私有 WebDAV 云存储     │
│  (Chrome/Edge/FF)│          零中转服务器 · 数据完全归你掌控       │ (坚果云/NAS/Nextcloud) │
└─────────────────┘                                                └────────────────────────┘
```
...
| 🛡️ **双轨快照与容灾配额** | **本地时光机快照 (IndexedDB)** 与**云端多版本备份 (WebDAV)** 双重保护；**最低强制保底 5 份**，支持用户自定义配额与自动清理 |
